...
class CointegrationCalculator:
    ...
    def dataframe_wrapper(self, asset_list: list) -> pd.DataFrame:
        ...
        asset_list[0]: self.time_series_matrix[asset_list[0]].astype(float).values, 
        asset_list[1]: self.time_series_matrix[asset_list[1]].astype(float).values,
        asset_list[2]: self.time_series_matrix[asset_list[2]].astype(float).values})
    ...
    def find_cointegrated_assets(self) -> list:
        ...
        for triplet in triplets:
            ...
            optimal_cointegration_results, optimal_lag = best_johansen_model_finder(asset_dataframe=asset_dataframe, max_lag=6)

            cointegrated_assets_list.append(list_cointegrated_assets(triplet=triplet, cointegration_results=optimal_cointegration_results, optimal_lag=optimal_lag))

        filtered_list = list(filter(lambda x: x is not None, cointegrated_assets_list))
        return filtered_list
    

def likelihood_ratio_test(likelihood_of_the_null_pypothesis: float, likelihood_of_the_alternative_pypothesis: float, degrees_of_freedom: int):
    likelihood_ratio_stat = 2 * (likelihood_of_the_alternative_pypothesis - likelihood_of_the_null_pypothesis)
...
def best_johansen_model_finder(asset_dataframe: pd.DataFrame, max_lag: int):
    ...
    for lag in range(1, max_lag):
        cointegration_results = coint_johansen(asset_dataframe, det_order=1, k_ar_diff=lag)
        ...
        likelihood_ratio_stat, p_value = likelihood_ratio_test(likelihood_of_the_null_pypothesis, likelihood_of_the_alternative_pypothesis, degrees_of_freedom)
